...
public class ECPrivateKey {
    ...
    init(for publicSecKey: SecKey, privateSecKey: SecKey) throws {
        self.curve = .prime256v1
        self.curveId = curve.description
        self.stripped = true
        var error: Unmanaged<CFError>? = nil
        guard let pubBytes = SecKeyCopyExternalRepresentation(publicSecKey, &error) else {
            guard let error = error?.takeRetainedValue() else {
                throw ECError.failedNativeKeyCreation
            }
            throw error
